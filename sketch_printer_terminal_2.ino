...
/* ---------- Back-light timer ---------- */
const uint32_t  BL_TIMEOUT_MS = 60000UL;   // 無操作60 s で消灯
const uint8_t   BL_DUTY_ON    = 128;
const uint8_t   BL_DUTY_OFF    = 0;
bool     blOn        = false;
...
const uint8_t BL_CH = 0;        // バックライト専用 A0
const uint8_t CNT_CH = 1;       // カウント専用     A1
const uint8_t SENSOR_PINS[] = { A0, A1 };
const uint32_t COOLDOWN_MS[NUM_IR] = {
    0,                // BL_SENSOR_PIN (使わない)
    30000UL           // CNT_SENSOR_PIN
};
const uint16_t LOT_THR_ADC  = 320;        // Lot++ 閾値
const uint16_t BL_THR_ADC   = 160;        // バックライト点灯 閾値
const uint16_t HYST         = 20;
const uint32_t DWELL_MS     = 1000;       // 1s 閾値を超えた状態
...
inline void backlightOn() {
  analogWrite(BL_PIN, BL_DUTY_ON);
  blOn=true; blLastOn_ms=millis();
  }
inline void backlightOff() {
  analogWrite(BL_PIN, BL_DUTY_OFF);
  blOn=false;
}
...
void loop() {
  ...
  for (uint8_t ch = 0; ch < NUM_IR; ch++) {
    ...
    /* back-light (CH0) */
    if (ch == BL_CH){
      bool blNear = adc > BL_THR_ADC + HYST;
      bool blFar = adc < BL_THR_ADC - HYST;

      if (!objNearForBL && blNear) {
         backlightOn();
         objNearForBL = true;
      }
      else if (objNearForBL && blFar) {
        objNearForBL = false;
      }
      continue;
    }
    /* common Lot++ 判定 */
    bool lotNear = adc > LOT_THR_ADC + HYST;
    bool lotFar = adc < LOT_THR_ADC - HYST;
